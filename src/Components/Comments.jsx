...
const Comments = ({ post, isCommentsOpen, closeComments }) => {
  ...
  return (
    <div className={`sticky inset-0 md:relative w-full md:w-[30%] bg-zinc-800 px-6 transition-all duration-300 
            ${isCommentsOpen ? "min-h-[70vh] opacity-100 visible" : "h-0 opacity-0 invisible"} overflow-y-scroll no-scrollbar`}>
      ...
      {postData && (
        <div>
          {/* Comment Input */}
          <div className='sticky top-0 bg-zinc-800 py-2 px-3 '>
            {/* Close Button */}
            <button className="absolute top-4 right-4 text-white text-xl z-20" onClick={closeComments}>
              ✖
            </button>
            <form className='border border-white flex rounded-[25px] overflow-hidden mt-12' onSubmit={CommentHandler}>
